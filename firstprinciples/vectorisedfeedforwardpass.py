...
def simple_neural_network(num_of_layers, input_signal, weights, bias):
    for layer_num in range(num_of_layers - 1):
        ...
        # using numpy's element-wise matrix multiplication to implement the 'z^(l + 1) = W^(l)h^(l) + b^(l)' and 'h^(l+1) = f(z^(l+1))' 
        sigmoid_function_input_value = weights[layer_num].dot(input_to_next_node) + bias[layer_num]
        output_signal = sigmoidfunction(sigmoid_function_input_value)
